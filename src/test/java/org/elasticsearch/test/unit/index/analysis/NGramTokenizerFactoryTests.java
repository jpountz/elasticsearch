...
public class NGramTokenizerFactoryTests {
    ...
    public void testPreTokenization() throws IOException {
        ...
        assertThat(toTerms(new NGramTokenizerFactory(index, indexSettings, name, settings).create(new StringReader("Åbc déf g\uD801\uDC00f "))),
                equalTo(Arrays.asList("Åb", "Åbc", "bc", "dé", "déf", "éf", "g\uD801\uDC00", "g\uD801\uDC00f", "\uD801\uDC00f")));
        ...
        assertThat(toTerms(new NGramTokenizerFactory(index, indexSettings, name, settings).create(new StringReader(" a!$ 9"))),
                equalTo(Arrays.asList(" a", " a!", "a!", "a!$", "!$", "!$ ", "$ ", "$ 9", " 9")));
    }
    ...
    public void testPreTokenizationEdge() throws IOException {
        ...
        final Settings indexSettings = ImmutableSettings.EMPTY;
        Settings settings = ImmutableSettings.builder().put("min_gram", 2).put("max_gram", 3).put("token_chars", "letter,digit").build();
        assertThat(toTerms(new EdgeNGramTokenizerFactory(index, indexSettings, name, settings).create(new StringReader("Åbc déf g\uD801\uDC00f "))),
                equalTo(Arrays.asList("Åb", "Åbc", "dé", "déf", "g\uD801\uDC00", "g\uD801\uDC00f")));
        settings = ImmutableSettings.builder().put("min_gram", 2).put("max_gram", 3).put("token_chars", "letter,digit,punctuation,whitespace,symbol").build();
        assertThat(toTerms(new EdgeNGramTokenizerFactory(index, indexSettings, name, settings).create(new StringReader(" a!$ 9"))),
                equalTo(Arrays.asList(" a", " a!")));
    }

    private List<String> toTerms(TokenStream ts) throws IOException {
