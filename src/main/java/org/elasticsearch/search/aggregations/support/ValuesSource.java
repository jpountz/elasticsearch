...
public abstract class ValuesSource {
    ...
    public static abstract class Bytes extends ValuesSource {
        ...
        public static abstract class WithOrdinals extends Bytes implements TopReaderContextAware {
            ...
            public abstract RandomAccessOrds globalBytesValues();
            ...
            public static class FieldData extends WithOrdinals implements ReaderContextAware {
                ...
                @Override
                public RandomAccessOrds globalBytesValues() {
                    if (globalBytesValues == null) {
                        globalBytesValues = globalAtomicFieldData.getOrdinalsValues();
                    }
    ...
    public static abstract class Numeric extends ValuesSource {
        ...
        public Bits docsWithValue(int maxDoc) {
            if (isFloatingPoint()) {
                ...
                final SortedNumericDocValues bytes = longValues();
                if (DocValues.unwrapSingleton(bytes) != null) {
                    return DocValues.unwrapSingletonBits(bytes);
                } else {
                    return DocValues.docsWithValue(bytes, maxDoc);
                }
            }
        }
        ...
        public static class WithScript extends Numeric {
            ...
            static class LongValues extends SortedNumericDocValues {
                ...
                @Override
                public int count() {
                    return source.longValues().count();
                }

                @Override
                public long valueAt(int index) {
                    script.setNextVar("_value", source.longValues().valueAt(index));
                    return script.runAsLong();
                }
            }

            static class DoubleValues extends SortedNumericDoubleValues {
                ...
                @Override
                public int count() {
                    return source.doubleValues().count();
                }

                @Override
                public double valueAt(int index) {
                    script.setNextVar("_value", source.doubleValues().valueAt(index));
                    return script.runAsLong();
                }
            }
        }
    ...
    public static class WithScript extends Bytes {
        ...
        static class BytesValues extends SortedBinaryDocValues {
            ...
            @Override
            public int count() {
                return source.bytesValues().count();
            }

            @Override
            public BytesRef valueAt(int index) {
                BytesRef value = source.bytesValues().valueAt(index);
