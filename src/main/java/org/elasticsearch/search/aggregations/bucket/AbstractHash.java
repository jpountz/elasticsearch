...
abstract class AbstractHash {
    ...
    AbstractHash(long capacity, float maxLoadFactor) {
    ...
    protected abstract void resizeKeys(long capacity);

    /** Remove key at the given index and  */
    protected abstract long removeAndAdd(long index, long id);
    ...
    protected final void grow() {
        ...
        for (long i = 0; i < buckets; ++i) {
            ...
            if (id > 0) {
                final long newId = removeAndAdd(i, id - 1);
        ...
        for (long i = buckets; i < newBuckets; ++i) {
            ...
            if (id > 0) {
                ...
                final long newId = removeAndAdd(i, id - 1); // add it back
                assert newId == id - 1 : newId + " " + (id - 1);
            } else {
                break;
            }
