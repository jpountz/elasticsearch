...
public final class BytesRefOrdValComparator extends NestedWrappableComparator<BytesRef> {
    ...
    // for assertions
    private boolean consistentInsertedOrd(RandomAccessOrds termsIndex, long ord, BytesRef value) {
    ...
    private long ordInCurrentReader(RandomAccessOrds termsIndex, BytesRef value) {
        final long ord;
        if (value == null) {
            ord = RandomAccessOrds.NO_MORE_ORDS << 2;
        } else {
    ...
    @Override
    public FieldComparator<BytesRef> setNextReader(AtomicReaderContext context) throws IOException {
        final RandomAccessOrds termsIndex = indexFieldData.load(context).getOrdinalsValues();
        missingOrd = ordInCurrentReader(termsIndex, missingValue);
        assert consistentInsertedOrd(termsIndex, missingOrd, missingValue);
        FieldComparator<BytesRef> perSegComp = new PerSegmentComparator(sortMode.select(termsIndex, -1));
        currentReaderGen++;
        if (bottomSlot != -1) {
    ...
    final protected static long binarySearch(RandomAccessOrds a, BytesRef key) {
        return binarySearch(a, key, 0, a.getValueCount() - 1);
    }

    final protected static long binarySearch(RandomAccessOrds a, BytesRef key, long low, long high) {
        assert low != RandomAccessOrds.NO_MORE_ORDS;
